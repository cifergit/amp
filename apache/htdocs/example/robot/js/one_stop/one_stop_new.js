...
requirejs(['jquery','global','validator','windowalert'], function ($,global,validator,windowalert) {
    ...
    function init(){
        ...
        vali.bind({
            domId: 'demandDesc',   //需要校验的dom元素ID
            eventType: 'blur',     //触发验证的事件类型companyNameErrmsg
            msgId: 'demandDescErrmsg',
    ...
    function submitForm(){
        ...
        $('.sub_category_cur').each(function(){
            ...
            else if($(this).data('service_type_id')){
                serviceTypeArr.push($(this).data('service_type_id'));
            }
        });
        ...
        if(serviceTypeArr.length == 0){
            $('#serviceTypeErrmsg').show().html('您最少需要选择一个服务类型');
            window.scrollTo(0, 0);
            return false;
        }
        else {
            $('#serviceTypeErrmsg').hide().html('您最少需要选择一个服务类型');
        }
        ...
        var serviceTypeStr = serviceTypeArr.join(',');
        var demandDesc = document.getElementById("demandDesc").value;
        ...
        var reqUrl = '/onestop/add_onestop_new';
        $.ajax({
            type : "POST",
            dataType : "json",
            data : reqData,
            ...
            error : function (er){
                windowalert.simple({
                    ...
                    leftButtionCallback : function(){
                        location.href = '/onestop';
                    },
                    buttionLeftValue : '返回一站式服务',
